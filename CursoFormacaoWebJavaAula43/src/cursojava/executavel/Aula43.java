...
public class Aula43 {
	...
	/* Aula 43 - Classes e Atributos Estáticos */
	...
	public static void main(String[] args) {
		...
		for (int quantidade = 0; quantidade <= 2; quantidade++) {
			...
			// Método para percorrer Lista de Alunos pelas posições

			for (int posicao = 0; posicao < alunos.size(); posicao++) {

				Aluno aluno = alunos.get(posicao);
				
				// Condição para substituir um objeto dentro de uma Lista
				if (aluno.getNome().equalsIgnoreCase("Hildo")) {
					Aluno trocar = new Aluno();
					trocar.setNome("Aluno foi trocado");
					
					//Adicionando uma disciplina para o aluno substituido
					Disciplina disciplina = new Disciplina();
					disciplina.setDisciplina("Matemática");
					disciplina.setNota(98);
				
					
					//Trocando  Disciplinas
				...
				// Método para percorrer Lista de Disciplinas pelas posições
				for (int posicaoDisc = 0; posicaoDisc < aluno.getDisciplinas().size(); posicaoDisc++) {

					Disciplina disciplina = aluno.getDisciplinas().get(posicaoDisc);
